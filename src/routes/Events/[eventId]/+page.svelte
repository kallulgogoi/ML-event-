...
<script lang="ts">
	...
	function submitRSVP() {
		// Rsvp form dats to send  backend
...
<section class="min-h-screen bg-[#121212] pt-16 text-gray-100">
	...
	{#if showRSVPModal}
		<div
			class="fixed inset-0 z-50 flex items-center justify-center bg-transparent bg-opacity-75 p-4"
		>
			<div class="border-1 w-full max-w-md rounded-xl border-white bg-[#121212] p-6 shadow-2xl">
		...
		<div class="relative pb-12 pt-8">
			...
			{:else}
				...
				<div class="mt-8 space-y-8">
					...
					<!-- Gallery - Horizontal Scrollable Grid (2 per row) -->
					{#if event.gallery?.length > 0}
						<div class="mt-8">
							<h2 class="mb-6 text-2xl font-semibold text-[#FF7700]">Event Gallery</h2>
							<div class="relative">
								<div
									class="gallery-scroller scrollbar-hide flex snap-x snap-mandatory gap-6 overflow-x-auto pb-6"
								>
									{#each event.gallery as image, index}
										<div
											class="h-96 w-[calc(50vw-3rem)] flex-none snap-start overflow-hidden rounded-xl shadow-lg sm:w-[calc(50%-1.5rem)]"
										>
											<img
												src={`/${image}`}
